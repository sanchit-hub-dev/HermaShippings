...
// ================================================================
// CLOCK — FIX: use UTC methods so label matches actual UTC time
// ================================================================
function updateClock(){
  const now = new Date();
  const s = t => String(t).padStart(2,'0');
  const str = s(now.getUTCHours())+':'+s(now.getUTCMinutes())+':'+s(now.getUTCSeconds())+' UTC';
  ['sbarTime','splashTime','fleetTime'].forEach(id=>{
...
function goFleet(){ closePdfViewer(); document.getElementById('app').style.display='none'; enterFleet(); }
function goHome(){ closePdfViewer(); document.getElementById('app').style.display='none'; document.getElementById('fleetScreen').style.display='none'; document.getElementById('splash').style.display='flex'; }
...
async function deleteVessel(id, name){
  ...
  // FIX: also delete actual Drive files when deleting a vessel
  const vesselFiles = files.filter(f=>f.vesselId===id);
  for(const f of vesselFiles){
    if(f.driveFileId) await api('deleteFile', {driveFileId:f.driveFileId});
  }
...
function renderFilesList(shown){
  ...
  el.querySelectorAll('.dlbtn').forEach(btn=>{
    btn.onclick = async e=>{
      ...
      const driveId = btn.dataset.drive;
      setSyncUI('app','syncing');
      if(driveId) await api('deleteFile', {driveFileId:driveId});
...
async function getPdfArrayBuffer(url){
  ...
  if(driveMatch){
    ...
    const res = await fetchWithTimeout(fallbackUrl, { credentials: 'include' }, 30000);
    if(!res.ok) throw new Error('Drive PDF fetch failed: '+res.status);
